...
// Attempted: 2021-12-29 21:27:57 EST
...
void solve(){
	...
	FOR(i,0,n){
		...
		int fr = right.size() - right.order_of_key(a[i]);
		int fl = left.size() - left.order_of_key(a[i]);
		if(max(fr,fl) > 2*min(fr,fl)){
			ans++;
		}
		left.ins(a[i]);
	}
