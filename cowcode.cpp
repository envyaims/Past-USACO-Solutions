// Problem: Problem 3. Secret Cow Code
// Contest: USACO - USACO 2017 January Contest, Silver
// URL: http://www.usaco.org/index.php?page=viewproblem2&cpid=692
// Memory Limit: 256 MB
// Time Limit: 4000 ms
// Input: file cowcode.in  
// Output: file cowcode.out  
// Attempted: 2021-08-17 20:18:00 EST
...
char findchar(ll now, ll len){
	if(now < s.length()){
		return s[now];
	}
	if(now < len/2){
		return findchar(now,len/2);
	}
	else{
		return findchar((now-1) % (len/2),(len/2));
	}
}
