// Author: envy
// Problem: Problem 1. Censoring (Bronze) from USACO - USACO 2015 February Contest, Bronze
// URL: http://www.usaco.org/index.php?page=viewproblem2&cpid=526
// Limits: 4000 ms, 256 MB
// Attempted: 2021-09-12 17:57:56 EST
...
void doStuff(){
	string init, del; cin>>init>>del;
	string build;
	for(int i = 0; i < init.length(); i++){
		build += init[i];
		if(build.length() >= del.length() && build.substr(build.length() - del.length()) == del){
			build.resize(build.length() - del.length());
		}
	}
	print(build);
