...
// Memory Limit: 256 MB
// Time Limit: 4000 ms
// Input: file reststops.in  
// Output: file reststops.out  
// Attempted: 2021-08-19 19:55:30 EST
...
void doStuff(){
	...
	sort(all(stops),customsort);
	
	int distance = 0;
	ll ans = 0;
	// its more worth it to eat the ones with the most tastiness value
	for(int i = 0; i < n; i++){
		if(stops[i].F > distance){
			int bes = (stops[i].F - distance) * b;
			int john = (stops[i].F - distance) * f;
			ans += stops[i].S * (john-bes);
			distance = stops[i].F;
		}
	}
	print(ans);
