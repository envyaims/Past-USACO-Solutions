// Author: envy
// Problem: Problem 2. Rental Service from USACO - USACO 2018 January Contest, Silver
// URL: http://www.usaco.org/index.php?page=viewproblem2&cpid=787
// Limits: 4000 ms, 256 MB
// Attempted: 2021-09-12 08:54:09 EST
...
void doStuff(){
	...
	readarr(cows);
	sort(all(cows));
	vector<int> prefixcows(n+1);
	
	for(int i = 0; i < n; i++){
		prefixcows[i+1] = cows[i] + prefixcows[i];
	}
	...
	sort(all(milk),greater<pair<int,int>>());
	vector<int> value(m+1);
	vector<int> amount(m+1);
	for(int i = 0; i < m; i++){
		value[i+1] = value[i] + milk[i].S * milk[i].F;
		amount[i+1] = amount[i] + milk[i].S;
	}
	
	vector<int> sell(r);
	readarr(sell);
	sort(all(sell),greater<int>());
	vector<int> prefixsell(r+1);
	for(int i = 0; i < r; i++){
		prefixsell[i+1] = prefixsell[i] + sell[i];
	}
	int ans = 0;
	for(int i = 0; i <= n; i++){
		int moners = 0;
		// cows 0 to i inclusive will be sold!
		int curmilk = prefixcows[n] - prefixcows[i];
		auto it = upper_bound(all(amount), curmilk) - amount.begin();
		curmilk -= amount[it-1];
		moners += value[it-1];
		moners += curmilk * milk[it-1].F;
		moners += prefixsell[min(i,r)];
		ans = max(ans,moners);
